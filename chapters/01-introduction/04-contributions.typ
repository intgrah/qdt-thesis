...
- A formalisation of the "Build Systems à la Carte" framework in Lean 4, refined with dependent query types (`Val : Key -> Type`), a separation of inputs from queries, and a well-foundedness condition on the dependency relation. Three build systems (Busy, LessBusy, Shake) are proved correct: their results equal batch evaluation by construction.
