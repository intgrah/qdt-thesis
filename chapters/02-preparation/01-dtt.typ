...
We implement a dependent type theory in the tradition of Martin-Löf @martinlof1984. We assume familiarity with the simply typed lambda calculus and focus on what distinguishes our system.
...
- $delta$-reduction (definition unfolding): $quad c.{overline(ell)} equiv t quad$ when $Delta(c) = t$
- $zeta$-reduction (let inlining): $quad sans("let") thin x : A := a thin sans("in") thin b equiv b[a\/x]$
- $eta$-conversion: $quad f equiv lambda (x : A). f thin x quad$ at function type
- $iota$-reduction (inductive type computation rule): recursor applied to a constructor computes to the corresponding branch

The full relation includes congruence, symmetry, transitivity, and closure under conversion for types.

=== Universe polymorphism

Universes are non-cumulative: $sans("Type")_u$ does not belong to $sans("Type")_v)$ for $v > u$. Definitions may be parameterised by universe level variables:
...
- A recursor `Vector.rec.{v, u}` that eliminates a `Vector` into a motive type, which is dependent on the thing being eliminated, by providing a case for `nil` and a case for `cons`:
